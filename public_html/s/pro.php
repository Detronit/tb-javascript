...
$pass = 'drewnoqq';
$passDel = 'drewnoqq';
$connection = @mysqli_connect('mysql.cba.pl', 'fordddd', $pass)
or die('No connection to Database');
$db = @mysqli_select_db($connection, 'mhtgaming_c0_pl')
or die('Cannot connect to Database');
?>
<!DOCTYPE html>
<html>
<head>
    <title>Lista programow</title>
...
<div style="overflow: auto">
    <div style=" align-items: center; float: left; width: 300px">
        <ul>
            <li><a href="?sub=<?php echo $sub_asemb ?>">Asembler</a></li>
            <li><a href="?sub=<?php echo $sub_obektowe_cpp ?>">Obiektowe c++</a></li>
            <li><a href="?sub=<?php echo $sub_numeryczne ?>">Numeryczne</a></li>
    ...
    <div style=" float: right; width: 70%">
        ...
        if (isset($_GET['sub'])) {
            ...
            if($sub == $sub_asemb)
                $sub = 'Asembler';
            else if($sub == $sub_numeryczne)
                $sub = 'Numeryczne';
            else if($sub == $sub_obektowe_cpp)
                $sub = 'Obiektowe cpp';
            echo '<span style="color: gainsboro; font-size: large; text-align: center">' . $sub . '</span>';
        }
        ?>
    </div>
</div>
<div>
    ...
    if (isset($_POST['code'])) {
        if (isset($_GET['sub'])) {
            $c = mysqli_real_escape_string($connection, $_POST['code']);
            $o = $_POST['user'];
            if ($o == '')
                $o = '_brak';
            $date = new DateTime();
            ...
            $sub = mysqli_real_escape_string($connection, $_GET['sub']);

            $sql = "INSERT INTO `programs` ( `code`, `owner`, `type`, `date`) VALUES ('$c', '$o', '$sub', now())";
    ...
    if (isset($_POST['pass']) && isset($_GET['del'])) {
        $d = mysqli_real_escape_string($connection, $_GET['del']);
        $p = mysqli_real_escape_string($connection, $_POST['pass']);
        $sql = "DELETE FROM `programs` WHERE `id` =   " . $d;
        if ($p == $passDel) {
    ...
    if (isset($_GET['sub'])) {
        $sub = $_GET['sub'];
        $wynik = @mysqli_query($connection, "SELECT * FROM `programs` WHERE `type`=$sub ")
        or die('Error during selecting DB');

        /*
        wyświetlamy wyniki, sprawdzamy,
        czy zapytanie zwróciło wartość większą od 0
        Kod:<input type="text" name="code" id="code" size="100"/>
        */
        ...
        ?>
        <br>
        <form action="/pro.php?sub=<?php echo $sub; ?>" method="post">
            Tworca/Tytul:<br><input type="text" name="user" id="user" size="2"/><br>
            Kod:<br><TEXTAREA name="code" cols="160"></TEXTAREA><br>
            <input type="submit" value="Dodaj kod"/></form>
        <br>
        <br>
        <br>
        <?php
        ...
        if (mysqli_num_rows($wynik) > 0) {
            ...
            foreach($data as $r)
            {
                $s = htmlspecialchars($r['code'], ENT_NOQUOTES);
                echo "<div>" . $r['id'];
                echo "<span style='color: darkgoldenrod; margin: 20px'>" . $r['owner'] . "</span>";
                echo "<span style='float: right'>" . $r['date'] . "</span></div><br>";
                echo '<pre class="prettyprint linenums" id="quine" style="border:4px solid #88c; font-family: Consolas, DejaVu Sans Mono, monospace">' . $s . "</pre>";
                // echo '<a href="/s/pro.php?del='.$r['id'] .'">Usun</a><hr></br>';
                ?>
                <form action="/pro.php?del=<?php echo $r['id']; ?>" method="post">
                    Pass: <input type="password" name="pass" size="2"/>
